...
def main():
    ...
    try:
        #opening the 'Windspeed.pickle' file, which contains the information of the lidar scanner, generated by "lidar_data.py"
        with open("Windspeed.pickle", "rb") as pickle_file:
            lidar_data = pickle.load(pickle_file)
        reference = lidar_data["reference"]
        while True:
            #it is necessary to check whether the pickle.file contains data since the pickle might be opened at the same time by another program ("lidar_data.py").
